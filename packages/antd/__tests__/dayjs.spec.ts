...
import { formatDayjsValue, dayjsable } from '../src/__builtins__/dayjs'

test('dayjsable is usable', () => {
  expect(dayjs.isDayjs(dayjsable('2021-09-08'))).toBe(true)
  expect(
    dayjsable(['2021-09-08', '2021-12-29']).every((item) =>
      dayjs.isDayjs(item),
    ),
  ).toBe(true)
  expect(dayjsable(0)).toBe(0)
})
...
test('formatDayjsValue is usable', () => {
  ...
  expect(formatDayjsValue([undefined], 'YYYY-MM-DD', 'placeholder')).toEqual([
    'placeholder',
  ])
})
